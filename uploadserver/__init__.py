...
document.getElementsByTagName('form')[0].addEventListener('submit', async e => {
  ...
  uploadRequest.upload.onprogress = e => {
    document.getElementById('status').textContent = (e.loaded === e.total ?
      'Saving…' :
      `${Math.floor(100*e.loaded/e.total)}% [${Math.floor(e.loaded/1024)} / ${Math.floor(e.total/1024)}KiB]`
    )
  }
...
class PersistentFieldStorage(cgi.FieldStorage):
    ...
    # Modified version of the original .make_file() method (base copied from Python 3.10)
...
def receive_upload(handler: http.server.BaseHTTPRequestHandler, ) -> tuple[http.HTTPStatus, str]:
...
def check_http_authentication_header(handler: http.server.BaseHTTPRequestHandler, auth: str, ) -> tuple[bool, str]:
...
def check_http_authentication(handler: http.server.BaseHTTPRequestHandler) -> bool:
    """
    ...
    def flush_headers_interceptor(self):
        for i, header in enumerate(self._headers_buffer):
            if header[:15] == b'Content-Length:':
                length = int(header[15:]) + len(DIRECTORY_BODY_INJECTION) + len(
                    get_directory_head_injection(args.theme))
...
class SimpleHTTPRequestHandler(ListDirectoryInterception, http.server.SimpleHTTPRequestHandler):
    def log_request(self, code='-', size='-'):
        ...
        self.log_message('%s %s %s\n%s\n\n%s',
                         self.requestline, str(code), str(size), headers_str, body_text)

    def log_message(self, format, *args):
        message = format % args
        log_header = Text(f"{self.address_string()} - - [{self.log_date_time_string()}]", style="cyan")
        log_msg = Text(message, style="blue")
        console.print(log_header, log_msg)
...
def ssl_wrap(socket: socket.socket) -> ssl.SSLSocket:
    ...
    if server_root in server_certificate.parents:
        print(f'Server certificate "{server_certificate}" is inside web server root "{server_root}", exiting')
        sys.exit(3)
    ...
    try:
        ...
        print(f'Unable to load certificate "{server_certificate}", exiting\n\n'
              f'NOTE: Certificate must be a single file in .pem format. If you have multiple certificate files, such as Let\'s Encrypt provides, you can cat them together to get one file.')
        sys.exit(4)
    ...
    if args.client_certificate:
        ...
        if server_root in client_certificate.parents:
            print(f'Client certificate "{client_certificate}" is inside web server root "{server_root}", exiting')
            sys.exit(3)
